...
def filter_events_by_stage(
    events: List[dict], config: Config, stage: PipelineStage
) -> List[dict]:
    stage_name = config.get_stage_name(stage)
    return [e for e in events if e.get("cat") == stage_name]


def assert_stage_events(
    events: List[dict],
    config: Config,
    stage: PipelineStage,
...
def test_convert_in_order(trace_with_in_order, config: Config):
    ...
    assert_stage_events(
        events, config, PipelineStage.FETCH, [(1000, 100), (2000, 100), (3000, 1)]
    )
    assert_stage_events(events, config, PipelineStage.DECODE, [(1100, 50), (2100, 50)])
    assert_stage_events(events, config, PipelineStage.RENAME, [(1150, 50), (2150, 50)])
    assert_stage_events(
        events, config, PipelineStage.DISPATCH, [(1200, 100), (2200, 100)]
    )
    assert_stage_events(events, config, PipelineStage.ISSUE, [(1300, 100), (2300, 100)])
    assert_stage_events(
        events, config, PipelineStage.COMPLETE, [(1400, 100), (2400, 100)]
    )
    assert_stage_events(events, config, PipelineStage.RETIRE, [(1500, 1), (2500, 1)])
...
def test_convert_ilp(trace_with_ilp, config: Config):
    ...
    assert_stage_events(events, config, PipelineStage.FETCH, [(1000, 100), (1000, 100)])
    assert_stage_events(events, config, PipelineStage.DECODE, [(1100, 50), (1100, 50)])
    assert_stage_events(events, config, PipelineStage.RENAME, [(1150, 50), (1150, 50)])
    assert_stage_events(
        events, config, PipelineStage.DISPATCH, [(1200, 100), (1200, 100)]
    )
    assert_stage_events(events, config, PipelineStage.ISSUE, [(1300, 100), (1300, 100)])
    assert_stage_events(
        events, config, PipelineStage.COMPLETE, [(1400, 100), (1400, 100)]
    )
    assert_stage_events(events, config, PipelineStage.RETIRE, [(1500, 1), (1500, 1)])
...
def test_convert_pipelined(trace_with_pipelined, config: Config):
    ...
    assert_stage_events(
        ...
        config,
        PipelineStage.FETCH,
        [(137000, 500), (137000, 500), (137000, 500)],
    )
    assert_stage_events(
        events,
        config,
        PipelineStage.DECODE,
        [(137500, 500), (137500, 500), (137500, 500)],
    )
    assert_stage_events(
        events,
        config,
        PipelineStage.RENAME,
        [(138000, 1000), (138000, 1000), (138000, 1000)],
    )
    assert_stage_events(
        events,
        config,
        PipelineStage.DISPATCH,
        [(139000, 53500), (139000, 63500), (139000, 64000)],
    )
    assert_stage_events(
        events,
        config,
        PipelineStage.ISSUE,
        [(192500, 10000), (202500, 500), (203000, 500)],
    )
    assert_stage_events(
        events,
        config,
        PipelineStage.COMPLETE,
        [(202500, 1000), (203000, 1000), (203500, 1000)],
    )
    assert_stage_events(
        events, config, PipelineStage.RETIRE, [(203500, 1), (204000, 1), (204500, 1)]
    )
...
def test_convert_unordered(trace_with_unordered, config: Config):
    ...
    assert_stage_events(
        ...
        config,
        PipelineStage.FETCH,
        [(78500, 500), (78500, 500), (78500, 500), (78500, 500)],
    )
    ...
    assert_stage_events(
        ...
        config,
        PipelineStage.DECODE,
        [(79000, 500), (79000, 500), (79000, 500), (79000, 500)],
    )
    ...
    assert_stage_events(
        ...
        config,
        PipelineStage.RENAME,
        [(79500, 1000), (79500, 1000), (79500, 1000), (79500, 1000)],
    )
    ...
    assert_stage_events(
        ...
        [(137000, 1), (141000, 1), (141000, 1), (141500, 1)],
    )
