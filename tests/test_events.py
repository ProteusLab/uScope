...
def filter_events_by_category(
    events: List[dict], config: Config, category: Any
) -> List[dict]:
...
def assert_duration_events(
    events: List[dict], config: Config, stage: PipelineStage, expected_data: List[Dict]
):
    filtered_events = filter_events_by_category(events, config, stage)
    assert len(filtered_events) == len(
        expected_data
